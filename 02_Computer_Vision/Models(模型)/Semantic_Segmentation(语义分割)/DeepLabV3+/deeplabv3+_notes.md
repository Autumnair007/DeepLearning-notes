...
想象一下你有一张彩色图像，它有红、绿、蓝三个通道。标准的卷积操作会使用一个卷积核同时处理所有这些通道，然后输出一个通道。这意味着这个卷积核会同时“看到”并融合所有输入通道的信息。

**深度卷积**则采取了一种截然不同的方法。它不是用一个卷积核处理所有输入通道，而是**为每一个独立的输入通道分配一个单独的卷积核**。

具体来说：

- 如果你的输入有 $C_{in}$个通道（比如一张RGB图像有3个通道），那么你就会有 $C_{in}$ 个独立的深度卷积核。
- 每个卷积核只负责处理它对应的那个输入通道。比如，处理红色通道的卷积核就只关注红色通道，绿色通道的卷积核只关注绿色通道，依此类推。
- 至关重要的是，在这个阶段，**不同通道之间的信息是完全隔离的，没有进行融合**。深度卷积的输出仍然保持$C_{in}$ 个通道，每个输出通道都包含了其对应输入通道的空间特征信息。

**为什么这样高效？** 对于一个标准的 3×3 卷积核，如果它处理 $C_{in}$ 个输入通道，那么它实际的参数量是 3×3×$C_{in}$。而在深度卷积中，你有$C_{in}$ 个独立的 3×3 卷积核，所以总的参数量是 $C_{in}$×(3×3)。这相比于标准卷积在生成多个输出通道时所需的参数量（我们将在逐点卷积部分看到），实现了显著的减少。

#### 逐点卷积 (Pointwise Convolution)

在深度卷积之后，你得到了一组特征图，其中每个通道的空间信息都已独立处理。但是，**不同通道之间的信息融合**这一关键步骤仍未完成。这就是**逐点卷积**发挥作用的地方。

- 逐点卷积使用 **1×1 的卷积核**。你可能会觉得奇怪，1×1 的卷积核不就是只看一个像素点吗？没错，但它是在**所有通道的同一个空间位置上**进行操作。
- 假设你的深度卷积输出有 $C_{in}$ 个通道。一个 1×1 的逐点卷积核将横跨这 $C_{in}$ 个通道，在某个特定的空间位置上对所有 $C_{in}$ 个输入通道的值进行加权求和，从而产生一个输出值（对应一个输出通道）。
- 为了获得多个输出通道（比如 $C_{out}$ 个输出通道，这通常是你想要的结果），你只需应用 $C_{out}$ 个不同的 1×1 逐点卷积核。每个卷积核都会通过组合所有 $C_{in}$ 个输入通道的信息来生成一个输出通道。
...
3. **一个全局平均池化 (Global Average Pooling, GAP) 分支：**
   ...
   - 作用：对整个输入特征图进行全局平均池化，然后通过一个 1×1 卷积处理。
   - 目的：捕获图像的**全局上下文信息**，这对于区分场景中的大范围区域或理解整体场景语义至关重要。
   - 处理：池化后的结果会被上采样回与原始输入特征图相同的空间分辨率，以便后续拼接。
   
   >实际上，GAP是将整个特征图**压缩成一个单一的、代表全局信息的特征向量**（好比一张大图的“情绪”或“摘要”）。这个 1×1 的全局特征值，经过一个 1×1 卷积处理后，会被**简单地复制或广播到所有空间位置（不准确地讲可以叫“上采样”）**，直到与原始输入特征图具有相同的分辨率。这样做并不是为了重建像素细节，而是为了将图像的**全局上下文信息**（比如“这是个室内场景”或“这张图主要是森林”）注入到每个像素的特征表示中，使得最终的像素级预测能同时考虑到局部细节和整体场景的语义，从而提高分割的准确性。
...
- **解码器设计**: 实验表明，将低级特征（Conv2）通过`[1x1, 48]`卷积进行通道缩减，然后与上采样的编码器特征拼接，再通过两个`[3x3, 256]`卷积进行精炼，是最高效的设计。
- **ResNet-101骨干**: 添加解码器模块后，无论训练和评估的输出步长是多少，性能都有显著提升。例如，在`output stride = 16`时，mIoU从77.21%提升到78.85%。
- **Xception骨干**:
  - 使用Xception作为骨干比ResNet-101性能提升约2%。
  - 在ASPP和解码器模块中应用深度可分离卷积（SC），可将计算复杂度（乘加操作）显著减少33%到41%，同时保持相似的mIoU性能。
  - 在MS-COCO和JFT-300M数据集上进行预训练能进一步提升性能，最终在PASCAL VOC 2012测试集上达到89.0%的SOTA性能。
- **边界改进**: Trimap实验量化了边界处的改进。结果表明，解码器模块能够有效恢复边缘细节，使分割边界更锐利，因为其直接利用了低级特征。膨胀带越窄，性能提升越显著（mIoU提升高达5.4%）。
- **Cityscapes结果**: 在Cityscapes数据集上，Xception-65作为骨干，添加解码器后性能从77.33%提升至78.79%。最终模型在测试集上达到了82.1%的SOTA性能。
