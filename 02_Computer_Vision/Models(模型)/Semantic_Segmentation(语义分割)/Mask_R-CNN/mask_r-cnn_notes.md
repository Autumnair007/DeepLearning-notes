...
- **正负样本定义：**
  - **正样本 (Positive Samples)：**
    - 与任何真实边界框 (Ground Truth Box) 的 IoU（交并比，Intersection over Union）大于某个阈值（例如 0.7）的锚框。
    - 或者，与某个真实边界框的 IoU 最高的锚框（即使 IoU 小于 0.7）。
...
首先，将原图上的 RoI 坐标除以特征图的下采样倍数（16），得到它在特征图上的大致坐标：
...
**RoI Pooling 会直接对这些浮点坐标进行向下取整（或者四舍五入，取决于具体实现），将其转换为整数像素坐标。**
...
现在 RoI 在特征图上的量化后坐标是 (2, 4, 9, 12)。

这个量化后的区域的宽度是 (9−2+1)=8 个像素，高度是 (12−4+1)=9 个像素。

**步骤 3：将量化后的区域划分为 2×2 的网格，并进行第二次量化**

我们想把这个 8×9 的区域变成 2×2 的网格。

- 每个子网格的宽度：8/2=4 像素。
- 每个子网格的高度：9/2=4.5 像素。

**RoI Pooling 会对 4.5 再次进行量化（通常是向下取整）**，所以每个子网格的高度也变成 4 像素。这意味着最后一个子网格可能不完整或被舍弃了一部分。

**步骤 4：在每个子网格中进行最大池化**

现在我们有了 4 个 4×4 （或类似尺寸，取决于具体量化）的子网格。RoI Pooling 会在每个子网格中找到**最大的特征值**，作为该子网格的代表，最终形成一个 2×2 的输出特征图。

**RoI Pooling 的问题：**

从上述过程中可以看到，**两次量化**导致了原始 RoI 边界和特征图上的像素点无法完美对齐。我们失去了 `0.0625`、`0.0625`、`0.375` 和 `0.5` 的精确信息。这些看起来很小的偏差，累积起来就会导致最终的掩码预测不准确，出现“错位”或锯齿状边缘。

------

#### 2. RoI Align 的处理方式 (新方法)

现在我们来看 **RoI Align** 如何处理同样的 RoI `(2.0625, 4.0625, 9.375, 12.5)`：

**步骤 1：不进行量化！直接使用浮点坐标**

RoI Align **不会**对 RoI 在特征图上的浮点坐标 `(2.0625, 4.0625, 9.375, 12.5)` 进行取整。

**步骤 2：将区域划分为 2×2 的网格，网格边界也是浮点坐标**

- 这个 RoI 的实际宽度是 9.375−2.0625=7.3125
- 这个 RoI 的实际高度是 12.5−4.0625=8.4375

现在我们要把它分成 2×2 的网格。

- 每个子网格的宽度：7.3125/2=3.65625
- 每个子网格的高度：8.4375/2=4.21875

**注意：** 这里的宽度和高度都是浮点数，**不会取整**。这意味着每个子网格的边界也都是浮点坐标。

**步骤 3：在每个子网格中选择采样点 (也是浮点坐标)**

为了获取每个子网格的特征值，RoI Align 会在每个子网格中选择**固定的采样点**。通常，每个子网格会选择 4 个均匀分布的采样点（例如，将子网格进一步划分为 2×2 的小块，取每个小块的中心）。

例如，对于第一个子网格（左上角），它的范围是：

- x 轴从 2.0625 到 2.0625+3.65625=5.71875
- y 轴从 4.0625 到 4.0625+4.21875=8.28125

在这个子网格内，我们可以选择 4 个采样点，它们的坐标都是**浮点数**。例如，某个采样点可能是 `(3.5, 6.1)`。

**步骤 4：使用双线性插值获取采样点的特征值**

这就是最关键的一步！

- 当我们需要获取 `(3.5, 6.1)` 这个浮点坐标的特征值时，它不会正好落在特征图的整数像素中心上。
- **双线性插值**会找到它周围最近的 4 个整数像素点 (例如，` (3,6)`, `(4,6)`, `(3,7)`, `(4,7)` )。
- 然后，它会根据这些整数像素点的特征值，以及采样点 `(3.5, 6.1)` 到这 4 个像素点的相对距离，进行**加权平均**，从而“精确估算”出 `(3.5, 6.1)` 这个浮点坐标点的特征值。

**步骤 5：对每个子网格进行池化**

最后，将每个子网格内所有采样点通过双线性插值得到的特征值，进行**最大池化**或**平均池化**，得到该子网格的最终特征。
