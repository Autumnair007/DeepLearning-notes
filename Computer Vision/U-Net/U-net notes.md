...
![7bc7489c897940cd010f0c85747222ad](U-net notes.assets/7bc7489c897940cd010f0c85747222ad.png)
...
*   **重复模块:** 此路径也由一系列重复的模块组成。每个模块通常包含：
    *   **一个 $2 \times 2$ 上采样卷积 (up-conv 2x2):** 这通常是一个反卷积 (Deconvolution) 或转置卷积 (Transposed Convolution)。它将特征图的空间维度扩大一倍（例如从 $28 \times 28$ 到 $56 \times 56$），同时通道数减半（例如从1024到512）。可以理解为卷积的逆过程，它将输入特征图的每个像素“扩展”到一个更大的区域。
    *   **特征拼接 (Concatenation via Skip Connection):** 这是 U-Net 的核心创新之一。上采样后的特征图会与收缩路径中对应层（相同空间分辨率）的特征图进行**拼接 (copy and crop)**。拼接操作将收缩路径中保留的局部细节信息（高分辨率特征）与扩张路径中恢复的全局语义信息结合起来。这对于精确定位分割边界至关重要。编码器在下采样过程中会丢失一些细节信息，但保留了较好的语义信息。跳跃连接将编码器中较浅层的高分辨率特征（包含更多细节和定位信息）直接传递给解码器中较深层的对应部分，帮助解码器更好地恢复图像细节。
...
## 图例解释 (Legend for Diagram Elements)

*   **蓝色框 (blue box):** 代表一个多通道的特征图。框上方的数字表示通道数，框左下角的数字表示特征图的 $x-y$ 尺寸 (例如 $572 \times 572$)。
*   **白色框 (white box):** 代表通过跳跃连接从收缩路径复制过来的特征图。
*   **深蓝色箭头 (conv 3x3, ReLU):** 表示一个 $3 \times 3$ 的卷积操作，后面跟着 ReLU 激活函数。
*   **灰色箭头 (copy and crop):** 表示跳跃连接，将收缩路径的特征图复制并可能进行裁剪（以匹配扩张路径特征图的尺寸），然后进行拼接。
*   **红色向下箭头 (max pool 2x2):** 表示一个 $2 \times 2$ 的最大池化操作。
*   **绿色向上箭头 (up-conv 2x2):** 表示一个 $2 \times 2$ 的上采样卷积（反卷积或转置卷积）。
*   **青色箭头 (conv 1x1):** 表示一个 $1 \times 1$ 的卷积操作。
