...
*   **输入：** 图像嵌入 + 提示嵌入 + 一个学习的输出 token 嵌入（类似 [CLS] token）。
*   **架构：** 改进的 2 层 Transformer 解码器（灵感来自 DETR 和 MaskFormer）：
    *   **自注意力 (Self-Attention)：** 在提示 token 之间。
    *   **交叉注意力 (Cross-Attention)：** 提示 token -> 图像嵌入 (更新 token)。
    *   **点向 MLP (Point-wise MLP)：** 更新每个 token。
    *   **交叉注意力 (Cross-Attention)：** 图像嵌入 -> 提示 token (更新图像嵌入，注入提示信息)。
*   **位置信息：** 在注意力层中，图像嵌入会**添加位置编码**，提示 token 会**重新添加其原始嵌入（含位置编码）**，确保模型强烈依赖提示的几何位置和类型。
*   **上采样与预测：**
    *   更新后的图像嵌入用 2 层转置卷积上采样 4 倍 (相对于输入图像下采样 4 倍)。
    *   输出 token 再次与上采样后的图像嵌入做注意力。
    *   更新后的输出 token 通过一个小的 3 层 MLP，输出一个向量。
    *   该向量与上采样后的图像嵌入做**空间点积 (Spatial Point-wise Product)** 预测掩码（前景概率图）。

**[技术细节解析]**

解码器的设计是整个模型交互效率的保障。它只有 **2 层**，非常轻量。其核心是一个**双向的注意力更新机制**：
1.  提示 token 作为 Query，从图像嵌入 (Key/Value) 中提取相关视觉信息来更新自己。
2.  更新后的提示 token 再作为 Query，反过来去更新整个图像嵌入，将提示的意图广播到整个特征图。
这个过程迭代进行，使得提示信息和图像信息充分融合。最终，模型不是直接预测像素，而是预测一个与特征图大小相同的**权重图**，这个权重图与上采样后的特征图做点积，生成最终的掩码，这种方式比直接预测高分辨率掩码更高效。
